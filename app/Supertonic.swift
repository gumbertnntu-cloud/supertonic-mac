...
struct AddSampleSheet: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                Text("Аудиофайл — оптимум 10–15 сек, чистая речь без шума и музыки")
                ...
                Text("Один образец достаточно. Разная интонация (точка + вопрос + восклицание) помогает.")
                    .font(.system(size: 10))
                    .foregroundStyle(.secondary)
            }
            ...
            VStack(alignment: .leading, spacing: 16) {
                ...
                section(
                    ...
                    rows: [
                        ...
                        ("F5 (clone)", "F5-TTS. Отличный английский (RTF ~2–3× через MLX). Русский — иностранный акцент."),
                    ...
                    footer: "Для русского — XTTS. Для английского с максимальной скоростью — F5. Лицензии CC-BY-NC / CPML."
                )

                section(
                    title: "Образцы для cloning — рекомендации",
                    rows: [
                        ("количество", "1 файл достаточно. Доп образцы пользы не дают."),
                        ("длина", "10–15 сек — оптимум. <5 мало, >30 модель путается."),
                        ("чистота", "Без шума, музыки, эха. Один спикер."),
                        ("интонация", "Утверждение + вопрос + восклицание в одной фразе — лучше."),
                        ("фразы", "«Привет. Меня зовут [имя]. Сегодня я записываю образец голоса. Получается интересно — да?»"),
                    ],
                    footer: "Формат любой (WAV/MP3/OGG, любой sample rate, mono лучше stereo). Worker сам подгонит."
                )

                section(
                    title: "Ударения и паузы в XTTS",
                    rows: [
